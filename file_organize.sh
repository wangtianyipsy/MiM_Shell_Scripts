...
for SUB in ${subjects[@]}; do
	...
	rm *.nii
	rm *.json
	dcm2niix -ba n ${Subject_dir}/Raw/MRI_files/LOCALIZER_0001
	...
	cd ${Subject_dir}/Raw/MRI_files/EP2D_DIFF_5B0_DISTMAP_PA_FLIPPED_0045
	rm *.nii.gz*
	rm *.nii
	rm *.json
	#dcm2nii -a n -i y -r n -x n ${Subject_dir}/Raw/MRI_files/EP2D_DIFF_5B0_DISTMAP_PA_0044
	dcm2niix -ba n ${Subject_dir}/Raw/MRI_files/EP2D_DIFF_5B0_DISTMAP_PA_FLIPPED_0045
	
	for nii_file in *.nii; do
		mv -v ${nii_file} "${nii_file:52:10}.nii"
		cp ${nii_file:52:10}.nii ${Subject_dir}/Processed/MRI_files/03_Fieldmaps/Fieldmap_dti;
	done
	for json_file in *.json; do
		mv -v ${json_file} "${json_file:52:10}.json"
		cp ${json_file:52:10}.json ${Subject_dir}/Processed/MRI_files/03_Fieldmaps/Fieldmap_dti;
	done
	####################################################################################
	# Resting State
